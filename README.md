# IntenCiv WhatsApp CRM

IntenCiv Diagnostics' self-hosted, multi-agent WhatsApp CRM for 7399000299 —
Reception, Sales, B2B, Operations and Management working one shared inbox
from individual logins, with an after-hours AI handover. See:

- `docs/authkey-integration-todo.md` — what's confirmed vs. still needed
  from Authkey before this is wired to a live number.
- The "WhatsApp Desk" architecture brief (published separately) for the
  full business context, roadmap and risk register.

This is a fork of [ArnasDon/wacrm](https://github.com/ArnasDon/wacrm)
(MIT-licensed — see `LICENSE`), adapted to send/receive WhatsApp messages
through **Authkey** instead of calling Meta's Cloud API directly, so
IntenCiv's existing Authkey setup (DLT SMS, OTP, current WhatsApp
integration) stays untouched. Everything below this point is wacrm's
original README, kept for reference — most of it (inbox, pipelines,
broadcasts, automations, AI assistant) still applies as-is; the WhatsApp
send/receive layer is what's being replaced.

---

# wacrm — CRM Template for WhatsApp (original upstream README)
